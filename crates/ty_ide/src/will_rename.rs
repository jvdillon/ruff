...
use ty_module_resolver::{
    Module, ModuleName, ModuleResolveMode, file_to_module, resolve_module_confident,
        ...
        match rename.kind {
            ...
            PathRenameKind::Directory => {
                ...
                if definition_files.len() != 1
                    || definition_files
                        .iter()
                        .any(|file| !file_is_within(db, *file, &old_path))
...
fn resolved_source_file(db: &dyn Db, name: &ModuleName) -> Option<File> {
    resolve_real_module_confident(db, name).map_or_else(
        || resolve_module_confident(db, name).and_then(|module| module.file(db)),
        |module| module.file(db),
    )
}
...
fn rename_edits_for_file(
    ...
    if contains_relevant_type_comment(source.as_str(), plan) {
        return None;
    }
    ...
    fn enter_node(&mut self, node: AnyNodeRef<'a>) -> TraversalSignal {
        ...
        match node {
            ...
            AnyNodeRef::StmtImportFrom(import) => {
                if import.level > 0
                    && let Some(rename) = self.moved_by
                {
                    match &rename.source {
                        RenameSource::Directory(_) => {}
                        RenameSource::File(_)
                            if rename.old_name.parent() != rename.new_name.parent() =>
                        {
        ...
        for alias in &import.names {
            ...
            let alias_parent = if let Some(module) = alias_module {
                ...
                if let Some((_, new_name)) = self.plan.rewrite_module(self.db, module) {
                    if old_name.parent().as_ref() == Some(&old_parent) {
                        if let Some(edit) = repeated_alias_edit(alias, old_name, &new_name) {
...
fn contains_relevant_type_comment(source: &str, plan: &RenamePlan) -> bool {
    source
        .lines()
        .filter_map(|line| line.split_once("# type:").map(|(_, comment)| comment))
        .any(|comment| {
            plan.rules.iter().any(|rule| {
                contains_identifier(comment, rule.old_name.first_component())
                    || contains_identifier(comment, rule.old_name.last_component())
            })
        })
}
    ...
    fn unsupported_rename_shapes_produce_no_edits() {
        assert_no_edits(
            &[
                ("/old_pkg/__init__.py", ""),
                ("/old_pkg/helper.py", ""),
                ("/old_pkg/old.py", "from . import helper\n"),
                ("/new_pkg/__init__.py", ""),
